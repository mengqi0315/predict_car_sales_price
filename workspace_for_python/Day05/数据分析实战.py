...
def crawl_data(html_str1):              # 这个方法用来将获取到的str格式的html进行xpath解析到rows这个列表中
    ...
    for index, el in enumerate(div_el):  # el数据类型是html文档类型
        ...
        company_info = child.xpath('normalize-space(//div[@class="cn"]/p[@class="msg ltype"]/text())')
        company_info = company_info if company_info else None
        desc_require = child.xpath('//div[@class="bmsg job_msg inbox"]/p/text()')
        desc_require = desc_require if desc_require else None
        row = (title, company,company_info, city, salary, time, exp, degree, fuli, desc_require)  # 将每一行数据封装到元祖中
        # print(row)
...
#         rows = crawl_data(html_str)                    # 调用函数爬取一页数据
#         csv_write('{}.csv'.format(keyword), 'a', rows)    #  写入到csv文件中

# 51job通过协程实现并发爬虫
def crawl_onepage(keyword,page):    #根据页码爬取当页信息
    url1 = 'https://search.51job.com/list/010000,000000,0000,00,9,99,{},2,{}.html?' \
           'lang=c&stype=&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99' \
           '&companysize=99&providesalary=99&lonlat=0%2C0&radius=-1&ord_field=0&confirmdate=9&fromType=' \
           '&dibiaoid=0&address=&line=&specialarea=00&from=&welfare='.format(keyword, page)
    html_str = get_html(url1,'gbk')    # 按照gbk的编码格式获取html字符串
    rows = crawl_data(html_str)      # 调用函数爬取数据(数据清洗)
    csv_write('{}.csv'.format(keyword), 'a', rows)  # 将数据写入到csv文件中


def crawl_gevent(keyword,start,end):
    head = ('职位', '公司', '公司信息', '工作地点', '薪资', '发布时间', '工作经验', '学历', '福利','岗位描述和任职资格')  # 第一行数据表头
    csv_write('{}.csv'.format(keyword), 'w', head)  # 调用刚才的csv_write方法
    tasks = []
    # 每循环一次将要爬虫一页的任务添加到协程组中
    for page in range(start, end + 1):
        task = gevent.spawn(crawl_onepage,keyword,page)   # 单个的协程任务
        tasks.append(task)
    gevent.joinall(tasks)

# crawl_gevent('python',1,5)
...
kw = {'kw': '数据分析师'}
kw = parse.urlencode(kw)[3:]    # 处理中文的关键字（必须使用url编码来处理）
print(kw)
crawl_gevent(kw,1,53)
