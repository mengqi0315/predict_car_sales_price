...
for index,item in enumerate(items):             # item 数据类型是html文档类型
     ...
     company = child_item.xpath('/div/span[@class="t2"]/a/@title') # 公司
     ...
     link_url = child_item.xpath('/div/p/span/a/@href')     #进入详情页的地址
     link_url = link_url[0] if link_url else None
     child_str = MyUtil.get_html(link_url)
     child_htm = etree.HTML(child_str)
     child_html = etree.ElementTree(child_htm)
     exp = child_html.xpath('//div[@class="jtag inbox"]/div/span/text()')
     print(exp)
